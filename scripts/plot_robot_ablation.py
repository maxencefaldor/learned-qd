...
metrics = metrics[metrics["descriptor"] != "zero"]
metrics = metrics[metrics["evo"].isin(["lqd", "ga"])]
...
for plot_idx, task in enumerate(robot_task.keys()):
	...
	if task == "arm":
		...
		task_metrics = metrics[
			...
			& ((metrics.descriptor == descriptor) | (metrics.descriptor == "random"))
		]

	# Plot each algorithm in order from evos_dict
	for evo in reversed(evos_dict):
		if evo not in task_metrics.evo.unique():
			continue
		...
		# Only plot no-descriptor variants for LQD
		if evo == "lqd":
			descriptor_metrics = evo_metrics[evo_metrics.descriptor == "random"]
			if len(descriptor_metrics) == 0:
				continue

			# Calculate statistics
			grouped = descriptor_metrics.groupby("evaluations")["fitness_max"]
			mean = grouped.mean()
			std = grouped.std()
			n = grouped.count()
			se = std / np.sqrt(n)
			...
			# Plot with dashed lines for ablation variants
			...
			if plot_idx == 0:
				lines.append(line[0])
				labels.append(f"{evos_dict[evo]} with random descriptors")
		...
		# Plot original descriptor variant for all algorithms
		if task == "arm":
			original_metrics = evo_metrics
		else:
			env_name = task.split("_")[0]
			descriptor = "_".join(task.split("_")[1:])
			original_metrics = evo_metrics[evo_metrics.descriptor == descriptor]

		if len(original_metrics) > 0:
			grouped = original_metrics.groupby("evaluations")["fitness_max"]
			mean = grouped.mean()
			std = grouped.std()
			n = grouped.count()
			se = std / np.sqrt(n)
...
plt.savefig("output/plot_robot_ablation.pdf", dpi=300, bbox_inches="tight")
plt.close()
