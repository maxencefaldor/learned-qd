...
class CMA_ES(DistributionBasedAlgorithm):
	...
	def _default_params(self) -> Params:
		...
		d_std = (
			1 + 2 * jnp.maximum(0, jnp.sqrt((mu_eff - 1) / (self.num_dims + 1)) - 1) + c_std
		)  # Eq. (55)
		chi_n = jnp.sqrt(self.num_dims) * (
			1.0 - (1.0 / (4.0 * self.num_dims)) + 1.0 / (21.0 * (self.max_num_dims_sq**2))
		)  # Page 28
		return Params(
			std_init=1.0,
			std_min=0.0,
	...
	def _tell(
		...
		# Update mean
		mean, y_k, y_w = self.update_mean(population, fitness, state.mean, state.std, params)
		# Cumulative Step length Adaptation (CSA)
		p_std = self.update_p_std(state.p_std, state.B @ ((state.B.T @ y_w) / state.D), params)
		...
		rank_mu = self.rank_mu(fitness, y_k, (y_k @ state.B) * (1 / state.D) @ state.B.T)
	...
	def update_p_c(self, p_c: jax.Array, h_std: bool, y_w: jax.Array, params: Params) -> jax.Array:
	...
	def rank_mu(self, fitness: Fitness, y_k: jax.Array, C_inv_sqrt_y_k: jax.Array) -> jax.Array:
		"""Compute the rank-mu update term for the covariance matrix."""
		w_o = fitness * jnp.where(
			fitness >= 0,
			1,
			self.num_dims / jnp.clip(jnp.sum(jnp.square(C_inv_sqrt_y_k), axis=-1), min=1e-8),
		)  # Eq. (46)
	...
	def update_C(
		...
		return (
			(1 + params.c_1 * delta_h_std - params.c_1 - params.c_mu * jnp.sum(params.weights)) * C
			+ params.c_1 * rank_one
			+ params.c_mu * rank_mu
		)  # Eq. (47)
