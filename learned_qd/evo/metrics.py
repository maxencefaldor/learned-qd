...
def metrics_fn(population):
	...
	valid = population.fitness != -jnp.inf
	std = jnp.std(novelty)
	sensitivity = 1.0
	scaling_factor = std / (std + sensitivity)
	novelty_standardized = jax.nn.standardize(novelty, where=valid)
	return {
		"fitness": population.fitness,
		"descriptor": population.descriptor,
		"novelty": novelty,
		"dominated_novelty": dominated_novelty,
		"qd_1": jnp.max(population.fitness, initial=-jnp.inf, where=valid) / (dominated_novelty + 1e-8),
		"qd_2": population.fitness / (novelty + 1e-8),
		"qd_3": population.fitness * jnp.exp(-novelty),
		"qd_4": population.fitness * jnp.exp(-scaling_factor * novelty_standardized),
	}
...
def metrics_agg_fn(metrics: dict) -> dict:
	...
	distance_to_mean = jnp.linalg.norm(metrics["descriptor"] - descriptor_mean[..., None, :], axis=-1)
	...
	return {
		...
		"dominated_novelty_max": jnp.nanmax(metrics["dominated_novelty"], axis=-1, initial=-jnp.inf, where=valid),
		"dominated_novelty_min": jnp.nanmin(metrics["dominated_novelty"], axis=-1, initial=jnp.inf, where=valid),
		...
		"qd_1_mean": jnp.nanmean(metrics["qd_1"], axis=-1, where=valid),
		"qd_2_mean": jnp.mean(metrics["qd_2"], axis=-1, where=valid),
		"qd_3_mean": jnp.mean(metrics["qd_3"], axis=-1, where=valid),
		"qd_4_mean": jnp.mean(metrics["qd_4"], axis=-1, where=valid),
