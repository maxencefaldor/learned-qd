...
def exponential(x: jax.Array, x_opt: jax.Array, r: jax.Array, q: jax.Array, num_dims: int) -> jax.Array:
...
def dixon_price(x: jax.Array, x_opt: jax.Array, r: jax.Array, q: jax.Array, num_dims: int) -> jax.Array:
	max_num_dims = x.shape[0]
	mask = jnp.arange(max_num_dims) < num_dims
	...
	out_1 = (z[0] - 1) ** 2
	out_2 = jnp.sum(mask[1:] * jnp.arange(2, max_num_dims + 1) * (2 * z[1:] ** 2 - z[: max_num_dims - 1]) ** 2)
	return out_1 + out_2, jnp.array(0.0)


def griewank(x: jax.Array, x_opt: jax.Array, r: jax.Array, q: jax.Array, num_dims: int) -> jax.Array:
	max_num_dims = x.shape[0]
	mask = jnp.arange(max_num_dims) < num_dims
...
def salomon(x: jax.Array, x_opt: jax.Array, r: jax.Array, q: jax.Array, num_dims: int) -> jax.Array:
	...
	return 1 - jnp.cos(2 * jnp.pi * jnp.linalg.norm(z * mask)) + 0.1 * jnp.linalg.norm(z * mask), jnp.array(0.0)
...
def levy(x: jax.Array, x_opt: jax.Array, r: jax.Array, q: jax.Array, num_dims: int) -> jax.Array:
	...
	out_2 = jnp.sum(
		(w - 1) ** 2 * (1 + 10 * jnp.sin(jnp.pi * w + 1) ** 2), where=jnp.arange(max_num_dims) < num_dims - 1
	)
	out_3 = jnp.sum((w - 1) ** 2 * (1 + jnp.sin(2 * jnp.pi * w) ** 2), where=jnp.arange(max_num_dims) == num_dims - 1)
