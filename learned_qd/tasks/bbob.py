...
class BBOBTask(Task):
	...
	def generate_random_rotation(self, key: jax.Array, num_dims: int) -> jax.Array:
		...
		mask = (jnp.arange(self.max_num_dims)[:, None] < num_dims) & (jnp.arange(self.max_num_dims)[None, :] < num_dims)
	...
	def random_index(self, key: jax.Array, num_dims: int) -> jax.Array:
		descriptor_ids = jax.random.choice(
			key,
			jnp.arange(self.max_num_dims),
			shape=(self.descriptor_size,),
			replace=False,
			p=jnp.arange(self.max_num_dims) < num_dims,
		)
		descriptor_params = jax.nn.one_hot(descriptor_ids, num_classes=self.max_num_dims)
