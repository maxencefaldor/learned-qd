	...
	def __init__(
		...
		if descriptor == "feet_contact":
			...
			if env_name == "ant":
				feet_idx = jnp.take(feet_idx, jnp.array([0, 3]))
			cog_idx = self.env.sys.link_names.index(COG_NAMES[env_name])
			self.get_descriptor = partial(get_feet_contact_velocity, feet_idx=feet_idx, cog_idx=cog_idx)
...
def get_position(env_states: brax.State, cog_idx: jax.Array) -> jax.Array:
	...
	return jnp.sum(env_states.pipeline_state.x.pos[..., cog_idx, :2], axis=-2, where=indicator[..., None])


def get_velocity(env_states: brax.State, cog_idx: jax.Array) -> jax.Array:
	mask = get_mask(env_states)
	return jnp.mean(env_states.pipeline_state.xd.vel[..., cog_idx, :2], axis=-2, where=mask[..., None])


def get_feet_contact_velocity(env_states: brax.State, feet_idx: jax.Array, cog_idx: jax.Array) -> jax.Array:
...
class MLP(nn.Module):
	...
	def __call__(self, obs: jax.Array) -> jax.Array:
		...
		kernel_init = self.kernel_init_final if self.kernel_init_final is not None else self.kernel_init
		hidden = nn.Dense(
			self.layer_sizes[-1],
