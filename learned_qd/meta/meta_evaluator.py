...
class MetaEvaluator:
	...
	def __init__(self, config: DictConfig) -> None:
		...
		for config_file in sorted([f for f in os.listdir(config_dir) if f.endswith(".yaml")]):
			...
			config_eval = hydra.compose(config_name=f"meta_evaluator/step/{config_name}").meta_evaluator.step
		...
		for config_file in sorted([f for f in os.listdir(config_dir) if f.endswith(".yaml")]):
			config_name = os.path.splitext(config_file)[0]
			config_eval = hydra.compose(config_name=f"meta_evaluator/final/{config_name}").meta_evaluator.final
	...
	def make_eval(self, config: DictConfig) -> Evolution:
		...
		def eval(params, key: jax.Array) -> dict[str, jax.Array]:
			...
			evo_state, metrics = jax.vmap(partial(evo.evolve, num_generations=config.num_generations, all_steps=True))(
	...
	def evaluate_step(self, params, key: jax.Array) -> dict[str, jax.Array]:
		...
		for name, eval in self.eval_step.items():
			...
			metrics = jax.tree.map(lambda x: jnp.mean(x[:, -1], axis=0), metrics_agg_fn(metrics))

			# Add task name prefix to each metric key
			for metric_name, metric_value in metrics.items():
				all_metrics[f"{name}_{metric_name}"] = metric_value
